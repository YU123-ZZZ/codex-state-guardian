...
set "GUARDIAN_PAYLOAD=%TEMP%\codex-state-guardian-%RANDOM%-%RANDOM%.ps1"
...
function Test-CodexStateJson {
  ...
  if ($item.Length -lt 100) {
    return @{ Ok = $false; Reason = "文件过小（$($item.Length) 字节）" }
  }
...
function Test-StateHealthy {
  ...
  if ($null -eq $config -or [int64]$config.size -lt 200) {
    $problems.Add('config.toml 缺失或过小')
  }
  if ([int]$State.enabledPluginCount -lt 1) {
    $problems.Add('没有发现已启用插件')
  }
  if (-not [bool]$State.globalStateJson.Ok) {
    $problems.Add(".codex-global-state.json 状态：$($State.globalStateJson.Reason)")
  }
  if ($null -eq $global -or [int64]$global.size -lt 100) {
    $problems.Add('.codex-global-state.json 缺失或过小')
  }
...
function Test-SameAsLatest {
  ...
  foreach ($path in $DuplicateComparisonPaths) {
    $currentItem = Get-StateItem -State $Current -RelativePath $path
    $backupItem = Get-StateItem -State $baseline -RelativePath $path
    if ($null -eq $currentItem -and $null -eq $backupItem) { continue }
    if ($null -eq $currentItem -or $null -eq $backupItem) { return $false }
    if ([bool]$currentItem.exists -ne [bool]$backupItem.exists) { return $false }
    if ((Get-ItemFingerprint $currentItem) -ne (Get-ItemFingerprint $backupItem)) { return $false }
  }
  return $true
}
...
function Test-RecentlyCreated {
  ...
  if ($Minutes -lt 1 -or $null -eq $Latest) { return $false }
  try {
    $createdUtc = [datetime]::Parse(
      [string]$Latest.Manifest.createdAtUtc,
      [Globalization.CultureInfo]::InvariantCulture,
      [Globalization.DateTimeStyles]::AssumeUniversal -bor [Globalization.DateTimeStyles]::AdjustToUniversal
    )
    return ((Get-Date).ToUniversalTime() - $createdUtc).TotalMinutes -lt $Minutes
  } catch {
    return $false
  }
}
...
function Remove-OldBackups {
  ...
  $old = @(Get-BackupDirectories -Root $Root | Select-Object -Skip $Keep)
...
function Invoke-Check {
  ...
  $restoreReasons = @()
  if (-not $health.Ok) { $restoreReasons += @($health.Problems) }
  if ($baselineProblems.Count -gt 0) { $restoreReasons += @($baselineProblems) }

  if ($restoreReasons.Count -gt 0) {
    Write-Warn "可能存在设置或聊天记录问题：$($restoreReasons -join '；')"
    if ($null -ne $latest) {
      $answer = if ($Force) { '确认' } else { Read-Host '是否恢复最新健康备份？如确认请输入：确认' }
      if (Test-Confirmed $answer) {
        Invoke-Restore -SelectedBackup $latest
      } else {
        Write-Info '已跳过恢复，真实文件没有改变。'
      }
    }
  } elseif ($health.Ok) {
    if (Test-SameAsLatest -Current $current -Latest $latest) {
      Write-Info '当前设置和聊天记录与最新健康备份一致，跳过重复备份。'
    } elseif ((-not $Force) -and (Test-RecentlyCreated -Latest $latest -Minutes $MinBackupIntervalMinutes)) {
      Write-Info "最新健康备份创建还不到 $MinBackupIntervalMinutes 分钟，跳过新备份。如需强制保存，请选择菜单 6，或使用参数 -强制。"
    } else {
  ...
  Remove-OldBackups -Root $BackupRoot -Keep $MaxBackups
}
